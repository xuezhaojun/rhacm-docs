...
* You need a cluster that you want to manage and Internet connectivity.
// 2. Is this a duplicate of the previous bullet?
...
// 3. "might be created" is vague. What does this mean? Is this a default
+
...
// 4. This is only needed if the cluster was previously managed by ACM. Should add this condition to the bullet.
...
// 5. Could we add a link to the Ansible Automation Platform docs?
...
// 6. What is a `clusterset-admin` specificly? Is this a role?
...
// 7. What is the recommended way here?
...
// 8. In what situations would this be necessary? And what needs to be configured?
...
// 9. Does this mean to make a kubeconfig file for the managed cluster? And we are using `oc` instead of `kubectl` in the next step.
...
// 10. What is this section for? Why we want to configure the cluster API address?
...
// 11. This is the first place mentioned hub cluster itself is named "local-cluster". I think maybe we could add the same note in the previous section to remind users that the hub cluster is named "local-cluster" by default.
...
// 12. Why it needs hours? Does that still take hours?
